...
import { X, Upload, Plus, Music, Music2, DollarSign, Tag, Activity, Image as ImageIcon, CheckCircle2, FileAudio, Trash2, ShieldCheck, ListMusic, Link as LinkIcon, AlertCircle } from 'lucide-react';
...
const AdminPortal: React.FC<AdminPortalProps> = ({ isOpen, onClose, onUpload, beats, onDelete, onToggleSold }) => {
  ...
  const formatDropboxLink = (url: string) => {
    if (url.includes('dropbox.com')) {
      // Convert standard Dropbox shared link to a direct raw link
      ...
      <div className="relative w-full max-w-5xl glass rounded-[3rem] overflow-hidden shadow-2xl border border-purple-500/20 flex flex-col h-[90vh]">
        <div className="p-10 border-b border-white/10 bg-gradient-to-r from-purple-900/20 to-transparent flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-gray-400 text-sm font-bold uppercase tracking-widest mt-1">Cloud Syncing Active</p>
          </div>
        ...
        {activeTab === 'upload' ? (
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-hide">
            <div className="bg-purple-600/10 border border-purple-500/20 p-6 rounded-3xl flex items-start gap-4">
              ...
              <div>
                <h4 className="font-black text-sm uppercase tracking-widest text-purple-400 mb-1">PRO TIP: CLOUD STORAGE</h4>
                <p className="text-xs text-gray-400 leading-relaxed font-medium">Use **Persistent Links** (Dropbox, Drive, S3) to ensure your beats don't disappear when you close the tab. Paste your Dropbox link and we'll handle the conversion for you.</p>
              </div>
            </div>
            ...
            <div className="flex gap-4 p-2 bg-white/5 rounded-2xl border border-white/5 w-fit">
              <button type="button" onClick={() => setUploadMode('link')} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${uploadMode === 'link' ? 'bg-white text-black' : 'text-gray-500 hover:text-white'}`}>
                <LinkIcon size={14} /> Persistent Links
              </button>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input required type="number" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none" value={formData.bpm} onChange={e => setFormData({...formData, bpm: parseInt(e.target.value)})} />
                  </div>
                  <div className="space-y-2">
                ...
                {uploadMode === 'link' ? (
                  <div className="space-y-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-purple-500 uppercase tracking-widest">Dropbox / Cloud Audio URL</label>
                      <div className="relative">
                        <LinkIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-600" size={18} />
                        <input required type="url" placeholder="https://www.dropbox.com/s/..." className="w-full bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 py-4 focus:border-purple-500 outline-none font-bold text-sm" value={formData.audioUrl} onChange={e => setFormData({...formData, audioUrl: e.target.value})} />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Cloud Cover Art URL</label>
                      <div className="relative">
                        <ImageIcon className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-600" size={18} />
                        <input required type="url" placeholder="https://images.unsplash.com/..." className="w-full bg-white/5 border border-white/10 rounded-2xl pl-14 pr-6 py-4 focus:border-blue-500 outline-none font-bold text-sm" value={formData.coverArt} onChange={e => setFormData({...formData, coverArt: e.target.value})} />
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className={`relative border-2 border-dashed rounded-3xl p-10 transition-all flex flex-col items-center justify-center text-center gap-3 ${audioFile ? 'border-green-500 bg-green-500/5' : 'border-white/10 hover:border-purple-500/50'}`}>
                      <input type="file" accept="audio/*" onChange={handleAudioChange} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
              ...
              beats.map(beat => (
                <div key={beat.id} className="glass border border-white/5 p-6 rounded-3xl flex items-center justify-between gap-6 hover:border-white/10 transition-all">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => onToggleSold(beat.id)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${beat.isSold ? 'bg-green-600/10 border-green-500/20 text-green-500' : 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                      {beat.isSold ? 'Restock' : 'Mark Sold'}
                    </button>
                    <button onClick={() => onDelete(beat.id)} className="p-3 rounded-xl bg-red-600/10 border border-red-500/10 text-red-500 hover:bg-red-600 hover:text-white transition-all"><Trash2 size={20} /></button>
                  </div>
                </div>
              ))
            )}
