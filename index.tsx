...
const App = () => {
  const [siteConfig, setSiteConfig] = useState<SiteConfig & { useExternalStore?: boolean, externalStoreUrl?: string }>({
    ...
    useExternalStore: true, // Defaulting to external store as requested
    externalStoreUrl: "https://www.beatstars.com/jmendezbeatz"
  });
  ...
  const fetchGlobalData = async () => {
    ...
    try {
      ...
      if (configRes && configRes.producerName) {
        setSiteConfig(prev => ({ ...prev, ...configRes }));
      }
    } catch (e) {
      setGlobalBeats([]);
    }
  ...
  const handleUpload = (newBeatData: Omit<Beat, 'id' | 'producer'> & { producer?: string }, isCommunity = false) => {
    const newBeat: Beat = {
      ...
      producer: isCommunity ? (newBeatData.producer || 'Guest') : siteConfig.producerName,
      isSold: false,
        ...
        <>
          <nav className="fixed top-0 left-0 right-0 z-[60] bg-black/80 backdrop-blur-3xl border-b border-white/5 px-4 sm:px-8 py-4 flex items-center justify-between shadow-2xl">
            ...
            <div className="flex items-center gap-4">
              {hasUnsyncedBeats && !siteConfig.useExternalStore && (
          ...
          <section className="pt-40 pb-20 px-4 sm:px-8 relative overflow-hidden text-center">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-[0.3em] text-purple-400 mb-10">
                <Globe size={12} /> {siteConfig.useExternalStore ? 'Beatstars Global Integration' : 'Live Marketplace'}
              </div>
          ...
          <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-8 pb-48">
            {siteConfig.useExternalStore ? (
              <BeatstarsStore url={siteConfig.externalStoreUrl || "https://www.beatstars.com/jmendezbeatz"} />
            ) : (
              <>
                <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
                  <div className="flex items-center gap-4">
                    <Waves className="text-purple-500 animate-pulse" size={32} />
                    <h2 className="text-3xl font-black italic uppercase tracking-tighter">Inventory</h2>
                  </div>
          ...
          {isAdminPortalOpen && (
            <AdminPortal 
              ...
              config={siteConfig}
              onConfigUpdate={(newConfig) => setSiteConfig(prev => ({...prev, ...newConfig}))}
