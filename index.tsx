    ...
    <div className="relative mb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        <div className="space-y-6 text-lg text-gray-400 leading-relaxed font-medium">
          <p>
            J Mendez is a multi-genre producer and sound designer dedicated to pushing the boundaries of modern sonics. With a signature style that blends gritty urban textures with polished cinematic atmospheres.
          </p>
...
const App = () => {
  ...
  const downloadFile = (url: string, filename: string) => {
    if (url.includes('dropbox.com')) window.open(url.replace('raw=1', 'dl=1'), '_blank');
    else {
    ...
    <div className="min-h-screen bg-[#070707] text-white selection:bg-purple-500/50 flex flex-col pb-32">
      {isCheckoutSuccess ? (
        <div className="fixed inset-0 z-[100] bg-[#070707] flex flex-col items-center justify-center p-6 text-center">
          <div className="mb-10 text-green-500 shadow-[0_0_50px_rgba(34,197,94,0.3)] rounded-full animate-bounce"><CheckCircle2 size={100} /></div>
          ...
          <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/5 px-10 py-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-8">
              ...
              <button onClick={() => setIsCartOpen(true)} className="relative p-3 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 group transition-all">
                ...
                {cart.length > 0 && <span className="absolute -top-1 -right-1 w-6 h-6 bg-purple-600 text-[10px] font-black flex items-center justify-center rounded-full border-2 border-[#070707]">{cart.length}</span>}
              </button>
            </div>
          </nav>
            ...
            {activeView === 'marketplace' ? (
              <div className="max-w-7xl mx-auto px-10">
                <div className="max-w-4xl mb-24 relative">
                  ...
                  <div className="relative group w-full max-w-2xl mt-12">
              ...
              <div className="max-w-4xl mx-auto glass rounded-[2rem] p-4 shadow-2xl border border-white/10 relative overflow-hidden pointer-events-auto">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 w-1/3">
                    <img src={currentBeat.coverArt} className="w-12 h-12 rounded-xl object-cover" />
                    <div className="hidden sm:block truncate"><h4 className="font-black text-sm uppercase italic leading-none">{currentBeat.title}</h4></div>
                  </div>
                  <div className="flex items-center gap-8">
